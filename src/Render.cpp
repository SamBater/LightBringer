    ...
    for (int x = bb->left; x < bb->right; x++) {
      ...
      if (ylb::Triangle::inside(x + 0.5f, y + 0.5f, t)) {
        ...
        if (cam->mode == PROJECTION_MODE::PERSPECTIVE)
          ...
          depth = t.vts[0].sz() * t.cof.x + t.vts[1].sz() * t.cof.y +
                  t.vts[2].sz() * t.cof.z;
        //深度测试
...
void Render::start() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    sprintf(str, "%s delta_time:%.3lf tri:%d", title, delta_time.count(),
            delTriangles);
...
void Render::generate_shadow_map(Light *light) {
  ...
  for (int y = 0; y < size.y; y++) {
    for (int x = 0; x < size.x; x++) {
      ...
      float depth =
          (1 +
           frame_buffer->depth[static_cast<int>(y * scaleY * w + x * scaleX)]) *
          127;
...
bool Render::back_face_culling(Vertex &vt) { return false; }
